...
#pragma unroll
    ...
    __device__ __forceinline__ void ff_sppark_half_calc_kernel(
        const uint32_t *a_shared,
        const uint32_t *b_shared,
        uint32_t *result_shared,
        size_t n)
    {
        ...
        {
            const uint32_t *current_a = a_shared + i * LEN_A;
            uint32_t *current_result = result_shared + i * LEN_A;

            uint32_t temp_a[LEN_A];
            uint32_t temp_result[LEN_A] = {0};
...
#pragma unroll
            for (int j = 0; j < LEN_A; j++)
            {
                uint32_t carry = 0;
#pragma unroll
                for (int k = 0; k < LEN_B && (j + k) < LEN_A; k++)
                {
                    uint64_t temp = (uint64_t)temp_a[j] + b_shared[k] + temp_result[j + k] + carry;
                    temp_result[j + k] = temp & 0xFFFFFFFF;
                    carry = temp >> 32;
